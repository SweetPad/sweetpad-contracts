...
contract SweetpadCharity is AccessControl {
    ...
    uint256 public availableTokens = 1000000 ether;

    event ScheduleCreated(address to, uint256 amount, uint256 period);
    event Claimed(address user, uint256 amount);

    constructor(address admin, address sweetToken_) {
        require(sweetToken_ != address(0), "SweetpadCharity: Token address can't be zero");
    ...
    function createSchedule(
        ...
        require(to != address(0), "SweetpadCharity: User address can't be zero");
        require(amount > 0, "SweetpadCharity: Amount can't be zero");
    ...
    function claim() external {
        require(userInfo[msg.sender].amount > 0, "SweetpadCharity: Nothing to claim");
        require(block.number >= userInfo[msg.sender].lockedUntil, "SweetpadCharity: Too soon to claim");
