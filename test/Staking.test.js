...
describe("Staking", function () {
	...
	describe("UnstakeSWT function", function () {
		it("Should revert with 'At least 10.000 xSWT is required'", async function () {
			await sweetToken.connect(deployer).approve(staking.address, parseEther("40000"));
			await staking.connect(deployer).stakeSWT(parseEther("40000"), 182);
			await expect(staking.connect(deployer).unstakeSWT(0, parseEther("30000"))).to.be.revertedWith(
				"At least 10.000 xSWT is required"
			);
		});
		...
		it("Should revert with 'Wrong id'", async function () {
			await sweetToken.connect(deployer).approve(staking.address, parseEther("40000"));
			await staking.connect(deployer).stakeSWT(parseEther("40000"), 182);
			await expect(staking.connect(deployer).unstakeSWT(1, parseEther("30000"))).to.be.revertedWith("Wrong id");
		});
		...
		it("Should revert with 'Staked amount is Zero'", async function () {
			...
			await staking.connect(deployer).unstakeSWT(0, parseEther("40000"));
			await expect(staking.connect(deployer).unstakeSWT(0, parseEther("10000"))).to.be.revertedWith(
		...
		it("Should unstake partial", async function () {
			...
			await staking.connect(deployer).unstakeSWT(0, parseEther("10000"));
